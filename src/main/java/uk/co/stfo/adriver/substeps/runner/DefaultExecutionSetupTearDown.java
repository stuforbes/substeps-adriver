...
import uk.co.stfo.adriver.substeps.common.MutableSupplier;
import uk.co.stfo.adriver.substeps.configuration.WebdriverSubstepsConfiguration;
import uk.co.stfo.adriver.substeps.driver.DriverFactory;
import uk.co.stfo.adriver.substeps.execution.ExecutionContextSupplier;
...
public class DefaultExecutionSetupTearDown {
    ...
    private static final MutableSupplier<TestRun> testRunSupplier = new ExecutionContextSupplier<TestRun>(Scope.SUITE,
            "__TEST_RUN__");
    ...
    @BeforeAllFeatures
    public void beforeAllFeaturesSetup() {
        ...
        final DriverFactory factory = createWebDriverFactory();

        ExecutionContext.put(Scope.SUITE, DriverFactory.DRIVER_FACTORY_KEY, factory);
    }
    ...
    public void basePreScenarioSetup() {
        ...
        final DriverFactory factory = (DriverFactory) ExecutionContext.get(Scope.SUITE,
                DriverFactory.DRIVER_FACTORY_KEY);
        testRunSupplier.set(new TestRun(factory.createDriver()));
    }
    ...
    public void basePostScenariotearDown() {
        ...
        if (!WebdriverSubstepsConfiguration.shutDownWebdriver()) {
            ...
            doShutdown = false;
                ...
                && !testRun.hasPassed()
                && (!WebdriverSubstepsConfiguration.closeVisualWebDriveronFail() && WebdriverSubstepsConfiguration
                        .driverType().isVisual())) {
        ...
        if (doShutdown) {
            ...
            if (testRun != null) {
                System.out.println("testRunSupplier.get().hasPassed(): " + testRun.hasPassed());
            }
    ...
    private DriverFactory createWebDriverFactory() {
        final Class<? extends DriverFactory> webDriverFactoryClass = WebdriverSubstepsConfiguration
        ...
        try {
            ...
            throw new IllegalStateException(String.format("Failed to create WebDriverFactory %s.",
                    webDriverFactoryClass.getName()), ex);
